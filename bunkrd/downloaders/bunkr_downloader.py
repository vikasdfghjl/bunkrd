...
class BunkrDownloader(BaseDownloader):
    ...
    def _dump_debug_info(self, slug, data):
        ...
        try:
            ...
            logger.debug(f"Dumping API response data to {debug_file} for analysis")
            
            with open(debug_file, 'w') as f:
                json.dump(data, f, indent=2)
                # No need for explicit f.flush() or f.close() as the context manager handles this
        except Exception as e:
    ...
    def _get_encryption_data(self, slug):
        ...
        try:
            ...
            # and randomized user-agent rotation with context manager to ensure proper cleanup
            with self.make_api_request('post', BUNKR_VS_API_URL, json={'slug': slug}) as r:
